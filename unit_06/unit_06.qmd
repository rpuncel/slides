...
format:
  html: default
  revealjs:
    output-file: unit_06.revealjs.html
    show-notes: true
...
- __Problem:__ Newton's second theory seemed to work up to the precision of 17th-century instruments
...
__We cannot deduce the probability of our model because we do not know enough about the structure of the world__
...
- We observe data, $D$
- Given that this data occurs, we want the probability that our hypothesis $P(H|D)$ is __true__
...
- __Collective:__ a frame of observations that can happen over and over
- As number of observations approaches infinity, proportion of throws of the die that show a 3 is 1/6
- The probability is the long-run frequency of the event relative to all observations (or of 3's relative to all throws) - Called __objective probability__
...
- __Subjective Probability:__ The probability of a hypothesis - Allows for disagreement about what it is
- Reflects our lack of information

__So, what probabilities can we study?__
...
- __\textit{p}-value:__ The probability of getting data as extreme as our observations, assuming the null hypothesis is true
...
__Null hypothesis__- Default assumption, status quo, statement that data might overturn
- $H_\varnothing: \text{Usually } \mu=0$
- No effect
:::
...
__Alternative hypothesis__- Idea or alternative to status quo
- $H_a:$ Usually $\mu \neq 0$
- Some effect exists
:::
...
__A hypothesis test does not prove the null hypothesis.__
...
__Never accept the null hypothesis.__
...
- We want to know if Vitamin W has an effect.  so we write 
...
![](figures/normal_with_two_tails.png)
- __Null hypothesis__ : $\mu = 0$
- __Alternative hypothesis__ : $\mu \neq 0$
...
![](figures/normal_with_one_tail.png)
- __Null hypothesis__ : $\mu = 0$
- __Alternative hypothesis 1__ : $\mu > 0$
- __Alternative hypothesis 2__ : $\mu < 0$
...
__Then__: The t-test is asymptotically valid
...
__Invalid effect size metrics__- t-stat
- p-value
:::
...
__Valid effect size metrics__- Mean values
...
- __Example__ : The effect of angled bristles on tooth decay
