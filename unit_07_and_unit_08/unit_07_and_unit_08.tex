...
\begin{frame}
  ...
  \note[item]{To be more precise, is this evidence that SF'ans have more black outfits than NYers in expectation?}
...
\vspace{1.5cm}
 Is this evidence that San Franciscans have more black outfits than New Yorkers \textit{in expectation}? \end{frame}
...
\begin{frame}
  ...
  \note[item]{The bottom distribution has low standard deviation. Now it seems quite unlikely to get two sample averages 1.2 apart. The data seems inconsistent with this hypothesis.}
    \begin{center}
        \includegraphics[width = 0.8\linewidth]{figures/fashion_plot.png}
    \end{center}
\end{frame}
...
\begin{frame}[t]
  ...
  \begin{exampleblock}{Assumptions} 
    ...
    Assume $X \sim N(\mu_X, \sigma_X)$.  $Y \sim N(\mu_Y, \sigma_Y)$.  We know $\sigma_X$, $\sigma_Y$.
      ...
      \note[item]{What is the distribution of $\overline{X} - \overline{Y}$? Let's use fact that a difference of normal RVs is normal. but which normal?}
      \note[item]{$V[\overline{X} - \overline{Y}] = V[\overline{X}] + V[\overline{Y}] = \sigma_X^2/n_X +  \sigma_Y^2/n_Y$}
      \note[item]{$\overline{X} - \overline{Y} \sim N(0, \sqrt{\frac{\sigma_X^2}{n_X}+\frac{\sigma_Y^2}{n_Y}})$}
      \note[item]{
      We can standardize by dividing. $z = \frac{\overline{X} - \overline{Y}}{ \sqrt{\frac{\sigma_X^2}{n_X}+\frac{\sigma_Y^2}{n_Y}}} \sim N(0,1)$}
...
sample size & 50 & 50
 ...
 \note[item]{$z = \frac{\overline{X} - \overline{Y}}{ \sqrt{\frac{\sigma_X^2}{n_X}+\frac{\sigma_Y^2}{n_T}}} = \frac{12.1 - 13.3}{\sqrt{\frac{4}{50}+\frac{4}{50}}} = \frac{-1.2}{ \sqrt{\frac{4}{25}}}  = -3.0$}
...
\begin{frame}
  ...
  \note[item]{From this example, I hope you see that the two-sample z-test is not all that different from the one-sample tests we did earlier.}
  \note[item]{No matter what the test is, there's a general pattern that you follow...}
  
  Three steps:
  \begin{enumerate}
      \item Specify model, null hypothesis, rejection criterion
...
\item Substitute $s$ for both $s_X$ and $s_Y$.
  $$t =  \frac{\overline{X} - \overline{Y}}{\sqrt{\frac{s^2}{n_X}+\frac{s^2}{n_Y}}} $$
  \end{itemize}
...
Some authors recommend a two step process:
  \begin{enumerate}
  \item Use Levine's test for equal variances ($H_0: \sigma_X = \sigma_Y$)
